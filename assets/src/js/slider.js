...
const beforeSliding = (i) => {
  let isActiveItem = document.querySelector(
      ".ac-slider--view__slides.is-active"
    ),
    ...
    sliderViewItems = document.querySelector(
      `.ac-slider--view__slides:nth-child(${nextItem})`
    );

  // if nextItem is bigger than the # of slides
  if (nextItem > sliderLength) {
    sliderViewItems = document.querySelector(
      ".ac-slider--view__slides:nth-child(1)"
    );
  }
  ...
  if (nextItem == 0) {
    sliderViewItems = document.querySelector(
      `.ac-slider--view__slides:nth-child(${sliderLength})`
    );
  }
