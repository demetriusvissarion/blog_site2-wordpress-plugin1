...
class TestimonialController extends BaseController
{
	...
	public function submit_testimonial()
	{
		...
		$args = array(
			...
			'meta_input' => array(
				'_demetrius1_testimonial_key' => $data
			)
		);
	...
	public function setShortcodePage()
	{
		$subpage = array(
			array(
				...
				'menu_slug' => 'demetrius1_testimonial_shortcode',
	...
	{
		wp_nonce_field('demetrius1_testimonial', 'demetrius1_testimonial_nonce');

		$data = get_post_meta($post->ID, '_demetrius1_testimonial_key', true);
...
?>
		<p>
			<label class="meta-label" for="demetrius1_testimonial_author">Author Name</label>
			<input type="text" id="demetrius1_testimonial_author" name="demetrius1_testimonial_author" class="widefat" value="<?php echo esc_attr($name); ?>">
		</p>
		<p>
			<label class="meta-label" for="demetrius1_testimonial_email">Author Email</label>
			<input type="email" id="demetrius1_testimonial_email" name="demetrius1_testimonial_email" class="widefat" value="<?php echo esc_attr($email); ?>">
		</p>
		<div class="meta-container">
			<label class="meta-label w-50 text-left" for="demetrius1_testimonial_approved">Approved</label>
			<div class="text-right w-50 inline">
				<div class="ui-toggle inline"><input type="checkbox" id="demetrius1_testimonial_approved" name="demetrius1_testimonial_approved" value="1" <?php echo $approved ? 'checked' : ''; ?>>
					<label for="demetrius1_testimonial_approved">
						<div></div>
					</label>
				</div>
			</div>
		...
		<div class="meta-container">
			<label class="meta-label w-50 text-left" for="demetrius1_testimonial_featured">Featured</label>
			<div class="text-right w-50 inline">
				<div class="ui-toggle inline"><input type="checkbox" id="demetrius1_testimonial_featured" name="demetrius1_testimonial_featured" value="1" <?php echo $featured ? 'checked' : ''; ?>>
					<label for="demetrius1_testimonial_featured">
						<div></div>
					</label>
				</div>
			</div>
...
<?php
	...
	{
		if (!isset($_POST['demetrius1_testimonial_nonce'])) {
			return $post_id;
		}

		$nonce = $_POST['demetrius1_testimonial_nonce'];
		if (!wp_verify_nonce($nonce, 'demetrius1_testimonial')) {
			return $post_id;
		}
